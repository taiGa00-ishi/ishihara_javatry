...
public class Step06ObjectOrientedTest extends PlainTestCase {
    ...
    public void test_objectOriented_zoo() {
        ...
        // _/_/_/_/_/_/_/_/_/_/
        // done jflute 次回1on1にて (2025/10/20)
        
        // Animal よりも上に生き物的なクラスがいて、そっちの下に by いしはらさん
        // bark()とかは、生き物的なクラスに移動でもいいのでは!?
        // ↑とても良い発想。
        // 打ち消すようなオーバーライドは、現実まあまああるけど...
        // それは(わざとまあいいだろうって)辻褄が合わせをしていて、
        // 本来なら、いしはらさんの通り、作ってない概念があって、
        // それを表現して階層にした方がベターである。
        //
        // Zombieを状態として捉える考え方もできる。(いぬのぞんび)
        // 実装イメージ、Animalで boolean isZombie;
        //
        // 構造を作るというのは、実装のレールを提供するに等しい。
        // 構造が間違ってると、その後の実装がどんどんいびつになる。
        // そのリスクがあることを踏まえてしっかり概念の設計しないといけない。
        //
        // 構造を作るからには、構造の精度が高い状態で提供しないといけないし...
        // 一方で、構造をもっと柔軟に改善していける体制とかスキルが欲しい。
    }
}
